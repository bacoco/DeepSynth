"""
SQuAD FR Dataset Converter.

Converts French SQuAD dataset to DeepSynth format with:
- Quality indicators (excellent/good/medium/poor/unreadable)
- Short answer format (SQuAD answers are typically short spans)
- Full context preservation
- Streaming support

Dataset: https://huggingface.co/datasets/qwant/squad_fr
"""
...
def convert_squad_fr(
    split: str = "train",
    max_samples: Optional[int] = None,
    streaming: bool = True,
    target_resolution: str = "gundam",
) -> Iterable[dict]:
    """Convert SQuAD FR to instruction format with pre-generated images."""
    LOGGER.info(f"Loading SQuAD FR ({split} split, streaming={streaming})...")
    ...
    if max_samples:
        ...
        dataset = load_dataset("qwant/squad_fr", split=f"{split}[:{max_samples}]")
        LOGGER.info(f"✅ Downloaded {len(dataset)} samples in {time.time() - start_time:.1f}s")
    else:
        LOGGER.info(f"⏳ Downloading full SQuAD FR dataset...")
        dataset = load_dataset("qwant/squad_fr", split=split)
        LOGGER.info(f"✅ Downloaded {len(dataset)} samples in {time.time() - start_time:.1f}s")
        ...
        try:
            for idx, sample in enumerate(dataset):
                try:
                    ...
                    # Extract answer (SQuAD has list of answers, use first)
                    ...
                    # SQuAD contexts are typically short, so no extraction needed
                    ...
                    payload = {
                        ...
                        "metadata": {
                            ...
                            "answer_end_token": None,  # Not provided in SQuAD
