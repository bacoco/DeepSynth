...
class EfficientIncrementalUploader:
    ...
    def cleanup_uploaded_batches(self, batch_files):
        """Move uploaded batch files to archive directory"""
        print(f"  🧹 Cleaning up {len(batch_files)} batch files...")

        for batch_id, batch_file in batch_files:
            try:
                # Move to uploaded directory
                archive_file = self.uploaded_dir / batch_file.name
                shutil.move(str(batch_file), str(archive_file))
                print(f"    ✓ Archived batch {batch_id}")
            except Exception as e:
                print(f"    ✗ Failed to archive batch {batch_id}: {e}")
