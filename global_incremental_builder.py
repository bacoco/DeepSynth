...
class GlobalIncrementalBuilder:
    ...
    def get_global_progress(self):
        ...
        try:
            ...
            for name, subset, _, _, expected_count in self.sources:
    ...
    def save_global_progress(self, progress):
        ...
        try:
            ...
            metadata = {
                ...
                'total_expected': sum(count for _, _, _, _, count in self.sources)
            }
...
## Dataset Sources
{chr(10).join([f"- {name} {subset or ''}: {count:,} samples" for name, subset, _, _, count in self.sources])}
...
"""
    ...
    def process_dataset_incremental(self, name, subset, text_field, summary_field, expected_count):
        ...
        for idx in range(start_idx, len(dataset)):
            ...
            if len(batch_samples) >= batch_size:
                ...
                if success:
                    ...
                    batch_samples.clear()
                    print(f"📈 Progress: {idx + 1:,}/{len(dataset):,} ({(idx + 1)/len(dataset)*100:.1f}%) - {progress['total_samples']:,} total samples")
            ...
            if (idx + 1) % 1000 == 0:
                print(f"  📊 Processed {idx + 1:,}/{len(dataset):,} samples ({len(batch_samples)} in current batch)")
    ...
    def run_complete_pipeline(self):
        ...
        for name, subset, text_field, summary_field, expected_count in self.sources:
            success = self.process_dataset_incremental(name, subset, text_field, summary_field, expected_count)
            if not success:
