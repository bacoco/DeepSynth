...
docker-compose -f "${SCRIPT_DIR}/docker-compose.gpu.yml" down 2>/dev/null || true

# Start the service
echo "▶️  Starting model training service..."
docker-compose -f "${SCRIPT_DIR}/docker-compose.gpu.yml" up -d
...
for i in {1..10}; do
    if curl -s http://localhost:5001/api/health > /dev/null 2>&1; then
        ...
        echo "To stop: docker-compose -f ${SCRIPT_DIR}/docker-compose.gpu.yml down"
        exit 0
    fi
